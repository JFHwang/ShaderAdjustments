...
void main()
{
    ...
    // Ambience, strength of existing color
    float specular = texture2D(gColor, gl_TexCoord[0]).a;
	
	//If current color is completely transparent (black) ignore
    if(specular == 0.0) {
 ...
 //Position in relation to camera
	...
	vec3 viewPos = cameraPosition - position;
